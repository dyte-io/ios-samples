...
class MeetingRoomViewController: UIViewController {
    ...
    @IBAction func participatsAction(_: Any) {
        let storyBoard = UIStoryboard(name: "Storyboard", bundle: nil)
    ...
    @IBAction func videoToggleAction(_: Any) {
        if dyteMobileClient?.localUser.videoEnabled ?? false {
...
extension MeetingRoomViewController: MeetingDelegate {
    ...
    func onMeetingRoomJoined() {
        meetingTitleLabel.text = dyteMobileClient?.meta.meetingTitle
    ...
    func refreshList() {
        ...
        if dyteMobileClient?.localUser.audioEnabled ?? false {
        ...
        participantsStatusButton.setTitle("\(meetingViewModel?.participants.count ?? 0)", for: .normal)
        recordMeetingButton.isHidden = !(dyteMobileClient?.localUser.permissions.host.canTriggerRecording ?? false)
